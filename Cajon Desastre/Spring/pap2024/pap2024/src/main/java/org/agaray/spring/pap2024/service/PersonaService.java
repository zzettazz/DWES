...
public class PersonaService {
    ...
    public void save(
        ...
        if (idNace != null) {
            persona.setNace(paisRepository.getReferenceById(idNace));
        }
        if (idVive != null) {
            persona.setVive(paisRepository.getReferenceById(idVive));
        }
    ...
    public Persona login(String dni, String password) throws Exception {
        Persona persona = personaRepository.getByDni(dni);
        if (persona == null) {
            throw new Exception("El DNI " + dni + " no existe");
        }
        if (!(new BCryptPasswordEncoder()).matches(password, persona.getPassword())) {
            throw new Exception("La contraseña para el DNI " + dni + " es incorrecta");
        }
        return persona;
    }
